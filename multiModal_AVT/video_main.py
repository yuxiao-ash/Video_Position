...
from model import VideoNet, Reg_Loss
...
def train(model, criterion, train_loader, valid_loader, optimizer, scheduler, args):
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        while batch_idx < len(train_loader) - 1:
            batch_idx += 1 
            videos, targets, names = prefetcher.next()
        ...
        scheduler.step()
        torch.save({'epoch': epoch+1, 'model': model.module.state_dict(), 'optimizer': optimizer.state_dict(), 'scheduler': scheduler.state_dict()}, '{}/videonet_epoch{}.pth'.format(args.save_prefix, epoch))

        score = val(model, valid_loader, args)
        
        if score < best_score:
            torch.save({'model': model.module.state_dict()}, '{}/videonet_bestmodel.pth'.format(args.save_prefix))
            best_score = score
...
def val(model, valid_loader, args):
    ...
    with torch.no_grad():
        ...
        while batch_idx < len(valid_loader) - 1:
            batch_idx += 1
            videos, targets, name = prefetcher.next()
            out = model(videos)
...
if(__name__ == '__main__'):
    ...
    dataset_train = VideoDataset(args.data_root, 'train', img_size=(66, 88))
    dataset_valid = VideoDataset(args.data_root, 'val', img_size=(66, 88))
    ...
    if args.test:
        dataset_test = VideoDataset(args.data_root + 'data_A/test/', 'test')

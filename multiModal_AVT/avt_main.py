...
from model import AVTNet, Reg_Loss
...
if __name__ == '__main__':
    ...
    dataset_train = AVTDataset(args.data_root, 'train', feature_type='mfcc', feature_lenght=8616)
    dataset_valid = AVTDataset(args.data_root, 'val', feature_type='mfcc', feature_lenght=8616)
    ...
    valid_loader = torch.utils.data.DataLoader(dataset_valid, batch_size=args.batch_size, shuffle=False,
                                               num_workers=args.num_workers, pin_memory=True)
    if args.test:
        dataset_test = AVTDataset(args.data_root + 'data_A/test/', 'test')

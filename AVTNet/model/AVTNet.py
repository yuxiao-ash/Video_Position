...
class RPN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(1, 32, 3, 1, 1), nn.BatchNorm2d(32), nn.ReLU(), nn.MaxPool2d(2, 2))
        self.conv2 = nn.Sequential(nn.Conv2d(32, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(), nn.MaxPool2d(2, 2))
        self.conv3 = nn.Sequential(nn.Conv2d(64, 96, 3, 1, 1), nn.BatchNorm2d(96), nn.ReLU(), nn.MaxPool2d(2, 2))
        self.conv4 = nn.Sequential(nn.Conv2d(96, 128, 3, 1, 1), nn.BatchNorm2d(128), nn.ReLU(), nn.MaxPool2d(2, 2))
    ...
    def forward(self, x):
        ...
        x = self.conv4(x)
        x = self.pool(x)
        x = x.view(B, T, -1).contiguous()# b, t, c
        return x
...
class VideoNet(nn.Module):
    def __init__(self):
        ...
        self.back_end = nn.GRU(512 + 128, 1024, 2, batch_first=True, bidirectional=True, dropout=0.3)
        self.pool = nn.AdaptiveAvgPool1d(200)
        self.rpn = RPN()

    def forward(self, x, text_ROIs):
...
# class AVTNet(nn.Module):
#     def __init__(self):
#         super(AVTNet, self).__init__()
#         self.videoNet = VideoNet()
#         self.audioNet = AudioNet()
#         self.se = SElayer(4096 + 512)
#         self.conv = nn.Sequential(nn.Conv1d(4096 + 512, 1, 1, 1, 0), Mish())
#         self.fc = nn.Sequential(nn.Linear(200, 64), Mish(), nn.Linear(64, 1), nn.Sigmoid())

#     def forward(self, video, audio, text_ROI):
#         video_feature = self.videoNet(video)
#         audio_feature = self.audioNet(audio)
#         text_feature = text_ROI.view(text_ROI.size(0), text_ROI.size(1), -1).permute(0, 2, 1)
#         feature = self.se(torch.cat([video_feature, audio_feature, text_feature], dim=1))
...
class AVTNet(nn.Module):
    ...
    def forward(self, video, audio, text_ROI):
        video_feature = self.videoNet(video, text_ROI)
        audio_feature = self.audioNet(audio)
...
class AVTNet2(nn.Module):
    def __init__(self):
        super(AVTNet2, self).__init__()
        self.videoNet = VideoNet()
        self.audioNet = AudioNet()
        self.se = SElayer(4096)
        self.conv = nn.Sequential(nn.Conv1d(4096, 1024, 1, 1, 0), Mish(), nn.Conv1d(1024, 1, 1, 1, 0), Mish())
        self.fc = nn.Sequential(nn.Linear(200, 64), Mish(), nn.Linear(64, 1), nn.Sigmoid())
...
class AVTNet_YOLT(nn.Module):
    def __init__(self, grid_num=20):
        super(AVTNet_YOLT, self).__init__()
    ...
    def forward(self, video, audio, text_ROI):
        ...
        cut_feature = torch.gather(feature, 2, new_index)#.detach()

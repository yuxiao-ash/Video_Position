...
def work(work_id, datas, save_name):
    os.environ['CUDA_VISIBLE_DEVICES'] = gpus[work_id % len(gpus)]
    print('work{} start!'.format(work_id))
    for i, path in enumerate(datas):
        t_start = time.time()
        # print(path)
...
if __name__ == '__main__':
    ...
    # 处理训练集
    main(data_root=save_root, save_name='ocr_text_trn')
    merge_json(dir_path='./ocr_text_trn', save_path='ocr_text_trn.json')
    # 处理B榜测试集
    main(data_root=save_root, data_dir='data_C', save_name='ocr_text_tst_B')
    merge_json(dir_path='./ocr_text_tst_B', save_path='ocr_text_tst_B.json')
    #********************************************

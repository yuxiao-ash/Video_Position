    ...
    for epoch in range(args.max_epoch):
        ...
        while batch_idx < len(train_loader) - 1:
            ...
            if batch_idx % args.show_freq == 0:
                print('epoch:{}\tloss:{:.5f}\tsmth:{:.5f}\tETA:{:.5f}'.format(epoch, loss.item(), smooth_loss, (toc-tic)/(batch_idx+1)*(len(train_loader)-batch_idx)/3600.0))
        scheduler.step()

        score = val(model, valid_loader, args)
        
        if score < best_score:
            torch.save({'model': model.module.state_dict()}, '{}/videonet_bestmodel_epoch{}.pth'.format(args.save_prefix, epoch))
...
if(__name__ == '__main__'):
    ...
    dataset_train = VideoDataset(args.data_root, 'train')
    dataset_valid = VideoDataset(args.data_root, 'val')
